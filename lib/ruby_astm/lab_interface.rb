...
#require_relative "custom/cinzan_platform"
...
module LabInterface
  ...
  module ClassMethods
    def log(message)
      puts message
      puts "ruby_astm_log #{Time.now.to_i}: #{message}"
  ...
  def write_receive_date_to_file(receive_data)
    full_file_path ||= File.join root,'../poly_test',"#{Time.now}.txt"
    #full_file_path2 ||= File.join root,'../poly_test',"f2.txt"
    IO.write(full_file_path, receive_data)

     #File.open(full_file_path2, 'w') { |file| file.write(receive_data) }
  ...
  def write_bytes_to_file(bytes)
    ...
    begin
      ...
      puts "ON RECEIVE DATA"
      write_receive_date_to_file(data)
      ...
      puts "DATA: #{data}" #Données au format HL7 ou ASTM (ETX) HEAD
   
      byte_arr = data.bytes.to_a #Convertir en tableau de byte

      puts "BYTE ARR: #{byte_arr}"
      ...
      puts "DATA BYTES: #{self.data_bytes}" # Garder dans tableau 
      ...
      puts "DATA BUFFER: #{self.data_buffer}"
  ...
  def process_text(text)
      ...
      puts "SPLITED TEXT IS: #{text.split("\n")}"
  ...
  def process_type(line)
      puts "******************************************"
      puts "PROCESSING TYPE: #{line.type}"
      case line.type
      when "Hl7_Header"
